...
| **Live Server** | Ritwick Dey | `ritwickdey.LiveServer` | 로컬 웹 서버를 구동하여 HTML 결과 리포트를 실시간으로 확인 가능 |
...
## 📄 HTML 결과 리포트 확인 (Live Server)

`--format html` 옵션을 사용하여 생성된 HTML 리포트(기본 경로: `output/scores.html`)를 브라우저에서 확인하는 방법입니다.

1. VSCode 탐색기에서 생성된 HTML 파일(예: `output/scores.html`)을 찾습니다.
2. 해당 파일을 **우클릭**한 뒤 **`Open with Live Server`**를 선택하거나, 에디터 우측 하단의 **`Go Live`** 버튼을 클릭합니다.
3. 브라우저가 자동으로 열리며 리포트가 표시됩니다.

> **Codespaces 환경 주의**: `Go Live` 실행 후 우측 하단에 뜨는 팝업에서 `Open in Browser`를 클릭하거나, 하단 터미널 영역의 `Ports` 탭에서 포워딩된 주소를 확인하여 접근하세요.
...
8. `ritwickdey.LiveServer` 검색 후 설치
